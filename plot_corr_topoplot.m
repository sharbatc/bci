function plot_corr_topoplot(eye_movement_corr, eeglab_path, ses, trial, name)
...
title(sprintf('%s: horizontal eye movement corr. session:%i, trial:%i',name, ses, trial));

subplot(1,2,2);
topoplot(zscore(eye_movement_corr(2,:))', 'eeglab_chan64_2.elp');
title(sprintf('%s: vertical eye movement corr. session:%i, trial:%i',name, ses, trial));
